...
class FirstPage(object):
    ...
    def createPage(self):
        ...
        self.combo1 = Combobox(self.root, values=[''] + self.pm.find_sub_dir(self.pm.root_path), state="readonly", textvariable=self.combovar1)
        ...
        self.combo2 = Combobox(self.root, values=[''] + self.pm.find_sub_dir(self.pm.root_path), state="disabled", textvariable=self.combovar2)
        ...
        self.combo3 = Combobox(self.root, values=[''] + self.pm.find_sub_dir(self.pm.root_path), state="disabled", textvariable=self.combovar3)
        ...
        self.combo4 = Combobox(self.root, values=['', 'create new'] + self.pm.find_sub_dir(self.pm.root_path), state="disabled", textvariable=self.combovar4)
    ...
    def comcmd3(self, a=None):
        ...
        self.combo4['values'] = ['create new'] + self.pm.find_sub_dir(f"{self.pm.root_path}\\{self.pm.latitude}\\{self.pm.num1}\\{self.pm.num2}")
        return
    ...
    def TakeNewPicture(self):
        ...
        self.clear()
        SecondPage(self.pm, self.rootpath, self.root)

    def NewContext(self):
        self.pm.create_context(int(self.pm.find_sub_dir(f"{self.pm.root_path}\\{self.pm.latitude}\\{self.pm.num1}\\{self.pm.num2}")[-1])+1)
        self.combo4['values'] = ['create new'] + self.pm.find_sub_dir(f"{self.pm.root_path}\\{self.pm.latitude}\\{self.pm.num1}\\{self.pm.num2}")

    def LogOut(self):
        from .LoginPage import LoginPage
        self.clear()
        LoginPage(self.pm, self.root)
